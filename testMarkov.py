...
def computeTextModel(text):
  textModel = markovModel('unknown lang', 'unknown encoding')
  for n in [3]:
    textModel.setN(n)
...
def main(argv=None):
  ...
  filenames = glob.glob('models/*MM*.save')
  print 'Found %d saved models: %s' % (len(filenames), filenames)
  allModels = markov.getSavedModels(filenames)
  ...
  for sample in sampleTexts:
    ...
    print 
    (bestModel, bestRank) = markov.tryMatch(allModels, intext, note)
